-- lottorich.co.kr 등 외부 소스에서 2등 누적 배출 이력을 가져오기 위한 베이스라인 컬럼.
-- historical_first_prize_count와 동일한 방식: draw_history가 추적하는 최근 회차와는
-- 별도로, 그 이전 시점부터 알려진 누적 2등 배출 횟수를 저장한다.
alter table public.stores
  add column if not exists historical_second_prize_count integer not null default 0;

comment on column public.stores.historical_second_prize_count is
  'draw_history 추적 이전의 알려진 누적 2등 배출 횟수 베이스라인 (예: lottorich.co.kr 스냅샷)';
...
begin
  ...
  select
    ...
    coalesce(a.second_prize_count, 0) + s.historical_second_prize_count,
